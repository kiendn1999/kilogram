...
class FollowRepository {
  Future<List<String>> getFollowers(String userID) async {
    final response = await ServerOperation().getDataFromServer(
        "https://leanhhuy.herokuapp.com/users/$userID/follower");

    if (response.statusCode == 200) {
      List<String> idFollowers =
          List.from(jsonDecode(response.body)['follower']);
      return idFollowers;
    } else
  ...
  Future<List<String>> getFollowings(String userID) async {
    final response = await ServerOperation().getDataFromServer(
        "https://leanhhuy.herokuapp.com/users/$userID/following");

    if (response.statusCode == 200) {
      List<String> idFollowings =
          List.from(jsonDecode(response.body)['followings']);
      return idFollowings;
    } else
      return jsonDecode(response.body)['error']['message'];
  }
  ...
  Future<bool> actionFollow(String ownerID, String guestID) async {
    ...
    final response = await ServerOperation().postDataToServer(
        url, jsonEncode(<String, String>{'following': guestID}));
  ...
  Future<bool> checkFollowing(String idOwner, String idUser) async {
    String url =
        'https://leanhhuy.herokuapp.com/users/$idOwner/checkfollow/$idUser';
